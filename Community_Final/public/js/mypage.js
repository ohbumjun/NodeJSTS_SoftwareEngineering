...
const mypageSubController = class extends subController {
    constructor(postClassNm, cmtClassNm) {
        ...
        this.createClickHandler = (type) => (e) => {
            let editContentsDiv = getHtmlElemByClassNm(`${type}-edit-open`, e.currentTarget);
            let editBtnsDiv = getHtmlElemByClassNm(`${type}-edit-buttons`, e.currentTarget);
            let content = getHtmlElemByClassNm(`${type}-description`, e.currentTarget);
            let editContent = getHtmlElemByClassNm(`${type}-edit-input`, e.currentTarget);
            let contentId;
            // display change
            ...
            if (e.target.id == `${type}-insert`) {
                ...
                if (editContent && contentId)
                    fetchReqInst.editORdeleteContent(`/community/edit${type}`, { content: editContent.value, contentId: Number(contentId.innerText) });
            }
            // delete
